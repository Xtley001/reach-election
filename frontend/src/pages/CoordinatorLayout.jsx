...
const NAV=[
  {to:'/coordinator/dashboard', label:'Dashboard', icon:'dashboard'},
  {to:'/coordinator/voters',    label:'Voters',    icon:'voters'},
  {to:'/coordinator/sessions',  label:'Sessions',  icon:'messaging'},
  {to:'/coordinator/team',      label:'Team',      icon:'team'},
...
export default function CoordinatorLayout() {
  ...
  return (
    <div style={{minHeight:'100dvh',background:'var(--bg)'}}>
      <aside style={{position:'fixed',top:0,left:0,bottom:0,width:220,background:'var(--bg)',borderRight:'1px solid var(--border)',zIndex:90}} className="sidebar">
        ...
        <nav style={{flex:1,padding:'var(--space-3) var(--space-2)',overflowY:'auto'}}>
          {NAV.map(n=>(
            <NavLink key={n.to} to={n.to} className="sidebar-link" style={({isActive})=>({
              ...
              fontWeight:isActive?600:400,fontSize:'var(--text-sm)',marginBottom:2,
            })}>
