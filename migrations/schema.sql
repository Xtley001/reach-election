...
CREATE TABLE IF NOT EXISTS users (
  ...
  CONSTRAINT chk_director_no_zone     CHECK ((role = 'director' AND zone_id IS NULL) OR role != 'director'),
  CONSTRAINT chk_coord_agent_has_zone CHECK ((role IN ('coordinator','agent') AND zone_id IS NOT NULL) OR role = 'director')
);

-- director_id references users — add after users table exists
...
-- ─── Voter seeding tables (must come before voters) ───────────────────────────
...
CREATE TABLE IF NOT EXISTS voters (
  ...
  name               VARCHAR(200)       NOT NULL,
  phone              VARCHAR(20),                  -- NULL for seeded voters until claimed
...
RETURNS TRIGGER AS $$
BEGIN
  UPDATE voters SET current_status = NEW.status_set WHERE id = NEW.voter_id;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- ─── Nightly cleanup function (schedule with pg_cron) ─────────────────────────
